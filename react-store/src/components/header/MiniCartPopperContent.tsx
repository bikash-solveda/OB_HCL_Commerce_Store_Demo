...
const MiniCartPopperContent: React.FC<MiniCartPopperContentProps> = (props: any) => {
  ...
  const handleCartOnClick = () => {
    if (!isB2B) {
      ...
      if (!cart || !userId || userId === cart.buyerId) {
        handleClose();
        navigate(CART);
      } else {
        handleClose();
      ...
      <StyledMiniCartContent
        ...
        cartLinkLabel={
          !isB2B
            ...
            : !cart
            ? t("MiniCart.Actions.Cart")
            : !userId || userId === cart.buyerId // remmeber me there is no userId
            ? t("MiniCart.Actions.Cart")
        ...
        isOrderOwner={!cart ? true : !userId || userId === cart.buyerId}
        isB2B={isB2B}
      />
    </ClickAwayListener>
  );
