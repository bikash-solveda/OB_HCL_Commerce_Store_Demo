...
export const useProductB2BDetailsLayout = (widget: Widget, page: Page, props) => {
  ...
  const [descriptiveAttributeList, setDescriptiveAttributeList] = useState<ProductDetailAttribute[]>([]);
  ...
  const loginNotRequired = useMemo(() => loginStatus || !mySite?.isB2B, [mySite?.isB2B, loginStatus]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    inventoryavailabilityService
      ...
      .then((res) => {
        if (res.data.InventoryAvailability !== undefined && res.data.InventoryAvailability.length > 0) {
          console.log("res.data.InventoryAvailability.length = " + res.data);
          for (const inventory of res.data.InventoryAvailability) {
            inventoryStatus = inventory.inventoryStatus === "Available" ? true : false;
            console.log(
              "inventoryStatus = " + inventoryStatus + " inventory.inventoryStatus = " + inventory.inventoryStatus
            );
  ...
  useEffect(() => {
    getProductDetails();
  }, [contract, i18n.language]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    ...
    if (productData && productData.longDescription) {
      const tab = <LongDescription typographySX={{}} longDesc={productData.longDescription} />;
      tabs.push({ title: translation.productDetailDescription, tabContent: tab });
    }

    if (descriptiveAttributeList.length > 0) {
      const tab = <DescriptiveAttributes partNumber={productPartNumber} attributes={descriptiveAttributeList} />;
      tabs.push({ title: translation.productDetailProductDetails, tabContent: tab });
    }
    ...
    if (attachmentsList.length > 0) {
      ...
      const tab = <AttachmentB2BLayout {...{ attachmentsList }} />;
      tabs.push({ title: translation.productDetailAttachments, tabContent: tab });
    }

    setProductDetailTabsChildren(cloneDeep(tabs));
