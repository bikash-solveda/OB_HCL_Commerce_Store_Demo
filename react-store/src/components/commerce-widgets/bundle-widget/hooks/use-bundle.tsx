...
const getInventoryStatus = (iv: any, rowData: any, quantity: number, physicalStore = false) => {
  if (rowData.selectedSku?.type === "sku" || rowData.type === "sku") {
    const imgAvl = "/SapphireSAS/images/Available.png";
    const imgUnAvl = "/SapphireSAS/images/Unavailable.png";
    const avl = { src: imgAvl, text: "CommerceEnvironment.inventoryStatusOnline.Available" };
    const oos = { src: imgUnAvl, text: "CommerceEnvironment.inventoryStatusOnline.OOS" };
    const unAvl = { src: imgUnAvl, text: "CommerceEnvironment.inventoryStatusOnline.NA" };
    const avlp = { src: imgAvl, text: "CommerceEnvironment.inventoryStatusStore.Available" };
    const oosp = { src: imgUnAvl, text: "CommerceEnvironment.inventoryStatusStore.OOS" };
    const unAvlp = { src: imgUnAvl, text: "CommerceEnvironment.inventoryStatusStore.NA" };

    if (!iv.available) {
      return physicalStore ? oosp : oos;
    } else if (parseInt(iv.quantity) < quantity) {
      return physicalStore ? unAvlp : unAvl;
    } else {
      return physicalStore ? avlp : avl;
    }
  }
...
const useAddToCartButton = (rows, getCurrentContext, getValueForCell, tableState, headers, selectedStore) => {
  ...
  const someInvalid = rows.some((r) => {
    ...
    if (!isProductOrVariant(r) || r.selectedSku) {
      const id = isProductOrVariant(r)
        ? r.selectedSku.uniqueID + "_" + r.selectedSku.sellerId
        : r.uniqueID + "_" + r.sellerId;
      ...
      if (physicalStoreId) {
        ...
        const idp = isProductOrVariant(r)
...
const AvailabilityCell = ({ rowData, headers }) => {
  ...
  const selectAttr = { src: "", text: "productDetail.SelectAttributes" };
  ...
  const status = getInventoryStatus(iv, rowData, q) || selectAttr;
  const physicalStoreId = selectedStore?.id;
  const { statusp, ivp } = useMemo(() => {
    const rc: { statusp?: any; ivp?: any } = {};
    if (physicalStoreId) {
      const { selectedSku } = rowData;
      const { uniqueID } = selectedSku ?? rowData;
      const ivp = get(getCurrentContext(ts), `inventory.${uniqueID}_${physicalStoreId}`, {});
      Object.assign(rc, { statusp: getInventoryStatus(ivp, rowData, q, true), ivp });
    }
    return rc;
  }, [physicalStoreId, rowData, getCurrentContext, ts, q]);
...
const useBundle = (props) => {
  ...
  const _getItems = useCallback(
    (rows, headers, tableState) => {
      ...
      rows.forEach((r) => {
        ...
        if (n) {
          q.push(n);
          p.push(isProductOrVariant(r) ? r.selectedSku.partNumber : r.partNumber);
          i.push(isProductOrVariant(r) ? r.selectedSku.uniqueID : r.uniqueID);
        }
      });
  ...
  const addToCart = useCallback(
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [rows, getValueForCell, xltn, contract, dispatch, headers, payloadBase, tableState]
  );
  ...
  const productDetailTabsChildren: ITabs[] = useMemo(() => {
    const rc: ITabs[] = [];
