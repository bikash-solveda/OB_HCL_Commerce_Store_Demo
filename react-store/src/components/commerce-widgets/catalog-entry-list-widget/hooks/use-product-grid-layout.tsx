...
import { productMetaDataSelector, selectedPageOffsetSelector } from "../../../../redux/selectors/catalog";
import {
  productListSelector,
...
// parameters that should not be included in the query because they have special meaning to ES --
//   we filter them out here -- the main code block will add them as necessary if needed
const SPECIAL = {
  searchTerm: true,
  categoryId: true,
  [MANUFACTURER]: true,
};
const removeSpecialParams = (params: URLSearchParams): Record<string, any> => {
...
export const useProductGridLayout = (props: any) => {
  ...
  const metaData = useSelector(productMetaDataSelector);
  ...
  const productList = useMemo(() => {
    ...
    } else if (searchTerm && metaData?.espot) {
      rc = _productList.map((p) => ({ ...p, metaData }));
    }
    return rc;
  }, [breadcrumbs, _productList, categoryId, metaData, searchTerm]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  }, [categoryId, mySite, contract, selectedSellers, i18n.language]);
