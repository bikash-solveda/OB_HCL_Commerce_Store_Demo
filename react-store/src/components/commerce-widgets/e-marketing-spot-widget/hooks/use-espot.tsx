...
export const useESpot = (widget: Widget, page: Page): { eSpot: ESpotState; marketingSpotData: any } => {
      ...
      if (node instanceof Element && node.type === "tag") {
        if (node.name === "a") {
          ...
          if (content.contentUrl || href) {
            ...
            const to = content.contentUrl ? content.contentUrl : href; // prefer content's url over the <a>'s href
            ...
            const target = to.startsWith("http://") || to.startsWith("https://") ? "_blank" : "_self";

            rc = (
              <StyledLink {...{ to, onClick, key, testId, className, tabIndex, style, target, ...attrs }}>
  ...
  const eSpotCallRef = useRef<string>("");
  React.useEffect(() => {
    if (
      mySite &&
      page &&
      emsName &&
      eSpotCallRef.current !== i18n.language &&
      (page.tokenName !== "ProductToken" || breadcrumbs?.length > 0)
      // make sure has access to parent categoryId through breadcrumbs
    ) {
  ...
  }, [page, mySite, emsName, breadcrumbs, i18n.language]);
