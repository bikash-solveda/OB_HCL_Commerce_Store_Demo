...
import { currentContractIdSelector, eligibleContractsSelector } from "redux/selectors/contract";
...
const RequisitionListDetailsPage = (props: any) => {
  ...
  const allContracts = useSelector(eligibleContractsSelector);
  ...
  useEffect(() => {
    if (orderId && currency && allContracts?.length) {
      dispatch(
        FETCH_ORDER_DETAILS_ACTION({
          orderId,
          currency,
