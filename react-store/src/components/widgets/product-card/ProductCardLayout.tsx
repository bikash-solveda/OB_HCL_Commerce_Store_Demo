...
import { Product } from "_foundation/types/product";
...
export default function ProductCard(props: ProductCardProps) {
  ...
  const {
    ...
    name,
    thumbnail,
    attributes: productAttributes = EMPTY_ATTRIBUTES,
    seo,
    link,
    metaData,
  } = product;
  ...
  const onClick = useCallback(() => {
    const { enableGA, storeName } = mySite;
    if (enableGA) {
      const listerFlag = breadcrumbs.length > 0 ? true : false;
      AsyncCall.sendProductClickEvent(
        { product, index: null, listerFlag, breadcrumbs, sellers, storeName },
        { enableUA: mySite.enableUA, enableGA4: mySite.enableGA4 }
      );
    }
    if (metaData?.espot) {
      dispatch(CLICK_EVENT_TRIGGERED_ACTION({ type: "search", product, widget: "ProductCardLayout", signal }));
    }
  }, [breadcrumbs, signal, dispatch, metaData, mySite, product, sellers]);
