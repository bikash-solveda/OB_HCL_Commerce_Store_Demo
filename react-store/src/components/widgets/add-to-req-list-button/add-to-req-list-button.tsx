...
export const AddToRequisitionListButton = ({ disabled, addFn }) => {
  ...
  const getList = useCallback(async () => {
    ...
    try {
      ...
      const f = res.data.resultList.filter(
        ({ memberId, userRegistration }) =>
          memberId === userId ||
          (rememberMe && userRegistration?.firstName === firstName && userRegistration?.lastName === lastName)
      );
      setList(f);
    } catch (e) {
  ...
  }, [firstName, lastName, rememberMe, userId]);
