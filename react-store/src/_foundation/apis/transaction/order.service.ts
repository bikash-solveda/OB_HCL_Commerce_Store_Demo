...
const orderService = {
  ...
  */
  ...
  addCSROrderComments(parameters: any): AxiosPromise<any> {
    const siteInfo = getSite();
    const { storeId = siteInfo?.storeID, orderId, mode, responseFormat, body, ...options } = parameters;
    return orderApiInstance.orderAddCSROrderComments(storeId, orderId, mode, responseFormat, body, options);
  },
