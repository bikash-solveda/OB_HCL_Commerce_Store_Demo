...
const SEO: React.FC = (props: any) => {
  ...
  const { isB2B = false } = site;
  const routes = useMemo(() => (isB2B ? ROUTE_CONFIG.B2B : ROUTE_CONFIG.B2C), [isB2B]);
  const match = matchRoutes(routes, pathname)?.find(
    ({ route, pathname: pn }) => pn === pathname && route.path !== ROUTE_CONFIG.Other.path
  );
  const foundMatch = useMemo(() => match !== undefined && match !== null, [match]);
  const identifier = useMemo(() => {
    const _i = pathname.substring(1) || HOME;
    //Rest client is using URLSearchParams to encode the query parameters.
    return decodeURI(foundMatch ? _i.split(CHILD_ROUTE_SEPARATOR)[0] : _i);
  }, [foundMatch, pathname]);
