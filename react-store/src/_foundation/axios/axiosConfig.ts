...
import { ResponseErrorResponseData, ResponseErrorResponseDataError } from "_foundation/types/response-error";
  ...
  if (url) {
    ...
    if (path) {
      ...
      return _serviceList.some((s) => {
        const { skip = "", contains = [] } = serviceList[s];
        return (
          s === serviceName &&
          skip !== _method &&
          (isEmpty(contains) || (typeof data === "string" && contains.every((item) => data.includes(item)))) // check if the request has a body and this body contains the keyword that indicates on the service list
        );
      });
    }
  }
...
const transformResponse = (request: AxiosRequestConfig) => (data, headers) => {
  if (typeof data === "string") {
    const previewToken = storageSessionHandler.getPreviewToken();
    const requiresParse = isNumberParserRequiredService(request);
    let tokenAdded = false;

    const parsed = losslessParse(data, (key, value) => {
      //transform number
    ...
    if (requiresParse || tokenAdded) {
      data = parsed;
    } else {
      // return data parsed in its original form
...
const isUseSnackbarHandleError = (error: AxiosError<ResponseErrorResponseData, any>) => {
  if (error.config) {
  ...
  return !(error.isAxiosError && (error.code === "ERR_CANCELED" || error.response?.status === NOT_FOUND));
};
...
const processGuestPartialAuthError = async (error: AxiosError<ResponseErrorResponseData, any>) => {
  const errorMessage = get(error, "response.data.errors[0]", {} as ResponseErrorResponseDataError);
  ...
  if (
    ...
    const rememberMe = process.env.REACT_APP_PERSISTENT_SESSION?.toLowerCase() === "true" ? { rememberMe: true } : {};
    const payload = {
      query: {
        ...rememberMe,
      },
    ...
    await guestIdentityService
      ...
      .then((resp: AxiosResponse) => {
        ...
        dispatch(GUEST_LOGIN_FETCH_CART_ACTION({ url: error.config?.url, ...currentUser }));
      })
      .catch((err) => {
        throw err;
      });
...
const processGuestLogon = async (request: AxiosRequestConfig) => {
  if (!request.url || (request.url.indexOf(GUEST_IDENTITY) === -1 && request.url.startsWith(site.transactionContext))) {
    let currentUser = storageSessionHandler.getCurrentUserAndLoadAccount();
    if ((!currentUser || (!currentUser.WCToken && currentUser.isGuest)) && isUserRequiredService(request)) {
      const rememberMe = process.env.REACT_APP_PERSISTENT_SESSION?.toLowerCase() === "true" ? { rememberMe: true } : {};
      const payload = {
        query: {
          ...rememberMe,
      ...
      await guestIdentityService
        ...
        .then((resp: AxiosResponse) => {
          ...
          //guest shopper always remembered when persistent session enabled.
          dispatch(GUEST_LOGIN_SUCCESS_ACTION({ ...currentUser, ...payload }));
          dispatch(GUEST_LOGIN_FETCH_CART_ACTION({ url: request.url, ...currentUser, ...payload }));
        })
        .catch((error) => {
          throw error;
        });
  ...
  Axios.interceptors.request.use(
    async (request) => {
      ...
      if ((previewToken && previewToken[WC_PREVIEW_TOKEN]) || isNumberParserRequiredService(request)) {
        request.transformResponse = [transformResponse(request)];
      }
