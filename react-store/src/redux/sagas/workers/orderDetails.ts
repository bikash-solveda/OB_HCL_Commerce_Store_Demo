...
export function* getOrderDetails(action: any) {
  ...
  try {
    ...
    try {
      ...
      const allProducts = allContracts?.length ? orderItems.map(({ productId }) => productId) : undefined;
...
export const fetchOrderItemDetailsByIds = (param: any) => {
  ...
  const { allProducts, allContracts } = rest;

  if (allProducts?.length) {
    const chunks = chunk<string>(allProducts, 50);
    chunks.forEach((id) =>
      promiseArray.push(productsService.findProductsUsingGET({ ...paramBase, id, contractId: allContracts, query }))
    );
  } else {
